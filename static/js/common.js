...
function showError(title, text) {
  ...
  console.log('*** common.js showError (title, text): ' + title + ' ' + text);
...
function handleError(title, data) {
  console.log('*** common.js handleError Error: ' + JSON.stringify(data));

  if (data.status == 401) {
    // Authentication error. Redirect back to the log in screen.
    console.log('*** common.js handleError, status 402, redicting to /logout ');

    window.location = '/logout';
  } else if (data.status == 0) {
    // Server could not be reached from the request.
    // It could be blocked, unavailable or unresponsive.
    console.log('*** common.js handleError status 0 ... server could not be reached ...');

    showError(title, 'Server could not be reached. Please try again.');
  } else if (data.responseJSON) {
    // JSON error that can be formatted.
    console.log('*** common.js handleError dat.parseJson. calling showJsonError() - still in common.js');
